...
#define c1 2.0 //加速度因子一般是根据大量实验所得
#define c2 2.0
#define maxgen 20    // 迭代次数
...
void PSO_func(void)
{
  ...
  for (int i = 0; i < maxgen; i++)
  {
    for (int j = 0; j < sizepop; j++)
    {
      ...
      // 速度更新
      Vx[j] = Vx[j] + c1 * rand1 * (pbestx[j] - popx[j]) + c2 * rand2 * (gbest[0] - popx[j]);
      if (Vx[j] > Vmax)
        Vx[j] = Vmax;
      if (Vx[j] < Vmin)
        Vx[j] = Vmin;
      Vy[j] = Vy[j] + c1 * rand1 * (pbesty[j] - popy[j]) + c2 * rand2 * (gbest[1] - popy[j]);
      if (Vy[j] > Vmax)
        Vy[j] = Vmax;
      if (Vy[j] < Vmin)
        Vy[j] = Vmin;
